...
<%--  
 ***************************************************************************************
 * Description  :   Presentation Layer for Viewing Monthly Feedback
...
<body>
				...
				<form id="form1" method="get" action="fetchMonthlyFeedback.mvc">
							<select id="month1" name="month1"  onchange="document.getElementById('form1').submit();" >
			...
			<c:if test="${isEmpty!=null && month!=null && feedbackMonthly==null}">
				<h3 class="center">No Entries Found for the Month ${month}</h3>		
			</c:if>
<c:if test="${feedbackMonthly!=null}">
						<h3 align="center" class="heading">Monthly Feedback for the month of ${month} </h3>
							<table class="centered highlight striped">
								...
								<%
									int i = 0;
								%>
								<c:forEach items="${feedbackMonthly}" var="feedback">
									<tbody>
										<tr>
											<td><%=++i%></td>
											<td>${feedback[0]}</td>
