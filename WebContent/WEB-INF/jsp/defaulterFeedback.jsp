...
<%--  
 ***************************************************************************************
 * Description  :   Presentation Layer for Viewing Defaulter's Feedback
...
<body>
	<div class="input-field col s12" style="width: 300px !important">
		<form id="form3" method="get" action="fetchDefaulters.mvc">

			<select id="month3" name="month3" onchange="document.getElementById('form3').submit();">
	...
	<c:if test="${isEmpty!=null && month!=null && defaultersFeedback==null}">
		<h3 class="center">No Entries Found for the Month ${month}</h3>		
	</c:if>
	<c:if test="${defaultersFeedback!=null}">
							<h3 align="center" class="heading">Monthly Defaulter's Feedback for the month of ${month} </h3>
							<table class="centered highlight striped">
								...
								<%
									int i = 0;
								%>
								<c:forEach items="${defaultersFeedback}" var="feedback">
									<tbody>
										<tr>
											<td><%=++i%></td>
											<td>${feedback[0]}</td>
